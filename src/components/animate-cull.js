...
const smoothing = 0.3;
const windowSize = 16;
const binWidth = (44100/(2*windowSize));
//*******************
...
export default class AnimateCull extends Component {
  constructor(props){
    ...
    this.state = {
      ...
      "url" : process.env.PUBLIC_URL + '/assets/a-long-walk-to-somewhere-close-errlymixx.mp3',
      "loop" : true,
      "fadeIn" : 0,
      "fadeOut" : 0
    })
  ...
  componentDidMount() {
    ...
    this.state.player.connect(this.state.gain);
    ...
    return (
      ...
      <button onClick={this.handleClick}>hola</button>
        <Surface width={1000} height={800} >
          <Cull mouse={this.state.mouse} meter={this.state.meter} fft={this.state.fft}>
            {process.env.PUBLIC_URL + '/assets/cull/cull2.png'}
          </Cull>
        </Surface>
      </div>
    );
